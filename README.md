...
蓝盛代付产品 HTML Demo 的统一维护仓库。所有需求都在同一个产品 Demo 中持续更新既有业务页面。

本仓库只维护统一入口、产品导航及其正式业务页面源，不收纳需求专属 Demo、PRD、截图、Graphify 产物或历史归档。
...
- `scripts/`：构建与完整性检查
- `test/`：项目结构和构建测试
- `docs/业务页面迭代.md`：既有页面更新与真实新增页面的接入规则
...
新需求仍在上级产品资料库中按完整需求包维护；页面变更直接合入本仓库中对应的既有业务页面，不复制需求专属 Demo。
...
5. `src/pages/` 下所有正式页面源都必须登记；未登记文件视为遗留副本，不得继续维护。
6. 运行 `npm run check`，确认统一入口和既有页面正常后再提交和推送。
